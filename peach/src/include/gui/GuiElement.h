...
#define AddGuiElement(...) peach::___PropertyTree::___AddElement(___AddGuiElementFirstArgName((__VA_ARGS__)), __VA_ARGS__)
...
public:
    ...
    template <typename T> inline static void ___AddElement(const char* name, const T& value) { ___AddElement(name, const_cast<T&>(value)); }
    ...
    {
        auto it = mTypeMap.find(typeid(T).name());
        Assert(it != mTypeMap.end(), "Unkown base-like type with setter!");
...
private:
    ...
    template <typename T> inline static void RollOutMapLikeContainer(const char* name, const T& begin, const T& end)
    {
        if (ImGui::TreeNodeEx(name, ImGuiTreeNodeFlags_Framed)) {
            for (auto it = begin; it != end; ++it) {
                ___AddElement(fmt::format("[{}]", typeid(it->first).name()).c_str(), it->second);
            }
            ImGui::TreePop();
        }
    }
